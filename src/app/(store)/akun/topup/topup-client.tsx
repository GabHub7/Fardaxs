...
export function TopUpClient({ balance, recent }: { balance: number; recent: RecentTopup[] }) {
      ...
      <div className="px-4 py-5 space-y-5 animate-fade-in-up">
        ...
        {result.qrUrl && (
          <div className="rounded-[20px] p-5 flex flex-col items-center" style={{ background: MOCK.bgMuted, border: `1px solid ${MOCK.border}` }}>
            ...
            <img src={result.qrUrl} alt="QRIS" className="w-52 h-52 rounded-[12px] bg-white p-2" />
          </div>
        )}
  ...
  return (
    <div className="px-4 py-5 space-y-5 animate-fade-in-up">
      ...
      {recent.length > 0 && (
        <div>
          <p className="text-sm font-bold mb-2.5" style={{ color: MOCK.foreground }}>Riwayat Top Up</p>
          <div className="rounded-[16px] overflow-hidden" style={{ background: MOCK.bgMuted, border: `1px solid ${MOCK.border}` }}>
            {recent.map((t, i) => {
              const c = t.status === 'PAID' ? MOCK.success : t.status === 'PENDING' ? MOCK.warning : MOCK.destructive
              const label = t.status === 'PAID' ? 'Berhasil' : t.status === 'PENDING' ? 'Menunggu' : t.status === 'EXPIRED' ? 'Kedaluwarsa' : 'Gagal'
              return (
                <div key={t.id} className="flex items-center justify-between px-4 py-3" style={{ borderTop: i > 0 ? `1px solid ${MOCK.border}` : 'none' }}>
                  <div>
                    <p className="text-sm font-semibold" style={{ color: MOCK.foreground }}>{formatCurrency(t.amount)}</p>
                    <p className="text-[11px] mt-0.5" style={{ color: MOCK.foregroundMuted }}>
                      {t.payment_method} · {new Date(t.created_at).toLocaleDateString('id-ID', { day: 'numeric', month: 'short' })}
                    </p>
                  </div>
                  <span className="text-[11px] font-semibold px-2.5 py-1 rounded-full" style={{ background: `${c}1a`, color: c }}>
                    {label}
                  </span>
                </div>
              )
