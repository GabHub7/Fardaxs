...
const KEY_LABELS: Record<string, string> = {
  CASAKU_BASE_URL: 'Base URL',
  CASAKU_LICENSE_KEY: 'License Key',
  CASAKU_QR_ID: 'QR ID (Merchant)',
  CASAKU_WEBHOOK_SECRET: 'Webhook Secret',
  OKECONNECT_API_URL: 'API URL',
  OKECONNECT_MEMBER_ID: 'Member ID',
  OKECONNECT_PIN: 'PIN',
  OKECONNECT_PASSWORD: 'Password',
  OKECONNECT_PROXY_URL: 'Relay Proxy URL (opsional)',
  OKECONNECT_PROXY_SECRET: 'Relay Proxy Secret (opsional)',
}
...
export function EnvVarsClient({
  ...
  const webhookUrl = appUrl ? `${appUrl}/api/webhooks/casaku` : '/api/webhooks/casaku'
        ...
        return (
          ...
          >
            ...
            {group === 'CASAKU' && (
              ...
              >
                <Webhook size={16} style={{ color: 'hsl(var(--foreground-muted))' }} className="flex-shrink-0 mt-0.5" />
                <div className="min-w-0 flex-1">
                  <p className="text-xs font-medium mb-1" style={{ color: 'hsl(var(--foreground))' }}>
                    URL Webhook (daftarkan di dashboard Casaku.id)
                  </p>
                  <div className="flex items-center gap-2">
                    <code
                      className="flex-1 min-w-0 truncate text-xs font-mono px-2.5 py-1.5 rounded-[10px]"
                    ...
                    <button
                      ...
                      className="flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-[10px] flex-shrink-0"
                      style={{ background: 'hsl(var(--primary) / 0.1)', color: 'hsl(var(--primary))' }}
                    >
                      {webhookCopied ? <ClipboardCheck size={13} /> : <Copy size={13} />}
